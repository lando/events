<template>
  <main class="home" aria-labelledby="main-title">
    ...
    <div class="overlay">
      ...
      <div v-else-if="selector === 'events' && events.length > 0" class="listing" id="listing">
...
const selector = ref('events');
const router = useRouter();

// Helper to get marker
const getMarker = (lat, lng, active = false) => ({
  icon: {
    anchor: {x: 0, y: 0},
...
const setActive = id => {
  ...
  // Update marker
  event.marker = getMarker(event.geo.latitude, event.geo.longitude, event.active);
  // Set nav
...
onMounted(() => {
  setActive(decodeURIComponent(location.hash.substring(1)));
});
